...
void main() {
    ...
    vec3 pixel = vec3(0.5 / u_resolution, 0.0);

    float s1 = turbulence_fbm(coords + pixel.xzz, 4, 0.5, 2.0);
    float s2 = turbulence_fbm(coords - pixel.xzz, 4, 0.5, 2.0);
    float s3 = turbulence_fbm(coords + pixel.zyz, 4, 0.5, 2.0);
    float s4 = turbulence_fbm(coords - pixel.zyz, 4, 0.5, 2.0);
    ...
    vec3 hemi = mix(ground_color, sky_color, remap(normal.y, -1.0, 1.0, 0.0, 1.0));
    ...
    float phong_value = max(0.0, dot(vec3(0.0, 0.0, 1.0), r));
    phong_value = pow(phong_value, 32.0);

    specular += phong_value;

    vec3 base_color = mix(
        vec3(1.0, 0.25, 0.25),
        vec3(1.0, 0.75, 0.0), smoothstep(0.0, 1.0, noise_sample));

    vec3 lighting = hemi * 0.125 + diffuse * 0.5;

    color = base_color * lighting + specular;
...
class TurbulentWaterScene implements Scene {
